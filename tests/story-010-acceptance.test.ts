/**
 * Acceptance test for story-010: Docs and validation: daemon vs cron cost comparison
 * 
 * This test verifies all acceptance criteria have been met:
 * 1. src/daemon/spawner.ts exists and exports all required functions
 * 2. src/daemon/daemon.ts exists with proper shebang and executable logic
 * 3. src/daemon/daemonctl.ts exists with start/stop/status exports
 * 4. src/cli/cli.ts includes spawner commands and --scheduler flag
 * 5. src/installer/run.ts supports scheduler: 'cron' | 'daemon' parameter
 * 6. src/db.ts migration adds daemon_active_sessions table and runs.scheduler column
 * 7. README.md updated with daemon section explaining --scheduler flag and cost savings
 * 8. Cost calculation documented: daemon polling (30 queries/hr @ ~0 cost) vs cron polling (12 queries/hr @ 3-4k tokens each)
 * 9. All existing tests continue to pass (backward compatibility maintained)
 * 10. New daemon tests pass (unit + integration)
 * 11. Typecheck passes
...
describe("story-010 acceptance criteria", () => {
  it("1. src/daemon/spawner.ts exists and exports required functions", () => {
    ...
    const spawner = require("../src/daemon/spawner.ts");
    assert.ok(typeof spawner.peekAndSpawn === 'function', "should export peekAndSpawn function");
    assert.ok(typeof spawner.spawnAgentSession === 'function', "should export spawnAgentSession function");
    assert.ok(typeof spawner.cleanupStaleSessions === 'function', "should export cleanupStaleSessions function");
  });
  ...
  it("3. src/daemon/daemonctl.ts exists with start/stop/status exports", () => {
    ...
    const daemonctl = require("../src/daemon/daemonctl.ts");
    assert.ok(typeof daemonctl.startDaemon === 'function', "should export startDaemon function");
    assert.ok(typeof daemonctl.stopDaemon === 'function', "should export stopDaemon function");
    assert.ok(typeof daemonctl.getSpawnerStatus === 'function', "should export getSpawnerStatus function");
  });
